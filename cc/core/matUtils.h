...
static inline void matPutVal(cv::Mat mat, Napi::Value value, int r, int c) {
  mat.at<type>(r, c) = (type)value.ToNumber().Int32Value();
}

template <typename type>
static inline void matPutVal(cv::Mat mat, Napi::Value value, int r, int c, int z) {
  mat.at<type>(r, c, z) = (type)value.ToNumber().Int32Value();
}
 ...
 */
template <typename type>
static inline Napi::Value matGetVal(const Napi::Env& env, const cv::Mat& mat, int r, int c) {
  return Napi::Number::New(env, mat.at<type>(r, c));
}

template <typename type>
static inline Napi::Value matGetVal(const Napi::Env& env, const cv::Mat& mat, int r, int c, int z) {
  return Napi::Number::New(env, mat.at<type>(r, c, z));
}


template <typename type>
static inline Napi::Value matGetVal(const Napi::Env& env, const cv::Mat& mat, const int* idx) {
 ...
 */
template <typename type>
static inline Napi::Value matGetVec2(const Napi::Env& env, cv::Mat& mat, int r, int c) {
  Napi::Array vec = Napi::Array::New(env, 2);
  (vec).Set(0u, Napi::Number::New(env, mat.at<cv::Vec<type, 2>>(r, c)[0]));
  (vec).Set(1u, Napi::Number::New(env, mat.at<cv::Vec<type, 2>>(r, c)[1]));
  return vec;
}

template <typename type>
static inline Napi::Value matGetVec2(const Napi::Env& env, cv::Mat& mat, int r, int c, int z) {
  Napi::Array vec = Napi::Array::New(env, 2);
  (vec).Set(0u, Napi::Number::New(env, mat.at<cv::Vec<type, 2>>(r, c, z)[0]));
  (vec).Set(1u, Napi::Number::New(env, mat.at<cv::Vec<type, 2>>(r, c, z)[1]));
  return vec;
}

template <typename type>
static inline Napi::Value matGetVec2(const Napi::Env& env, cv::Mat& mat, const int* idx) {
  Napi::Array vec = Napi::Array::New(env, 2);
 ...
 */
template <typename type>
static inline Napi::Value matGetVec3(const Napi::Env& env, cv::Mat& mat, int r, int c) {
  Napi::Array vec = Napi::Array::New(env, 3);
  (vec).Set(0u, Napi::Number::New(env, mat.at<cv::Vec<type, 3>>(r, c)[0]));
  (vec).Set(1u, Napi::Number::New(env, mat.at<cv::Vec<type, 3>>(r, c)[1]));
  (vec).Set(2u, Napi::Number::New(env, mat.at<cv::Vec<type, 3>>(r, c)[2]));
  return vec;
}
